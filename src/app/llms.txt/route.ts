...
export async function GET() {
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://noob-incremental.wiki'
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'Noob Incremental Wiki'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(`> ${siteName} provides Noob Incremental guides, codes, progression tips, rune advice, and community resources.`)
  lines.push('')
